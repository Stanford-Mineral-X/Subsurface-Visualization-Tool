...
class VisualizationTool(HasTraits):
    ...
    def update_visualization(self):
        ...
        axes = mlab.axes(outline, color=(0, 0, 0),
                         xlabel='X', ylabel='Y', zlabel='Z')
        axes.label_text_property.color = (0, 0, 0)
        axes.title_text_property.color = (0, 0, 0)
        axes.axes.label_format = '%g'  # To show numerical labels
    ...
    def plot_standard(self):
        ...
        # Add title
        mlab.title(f"{self.title} - Standard View",
    ...
    def plot_slicing(self):
        ...
        # Display a single slicing plane
        plane = mlab.pipeline.image_plane_widget(
            self.plot_data, plane_orientation='z_axes', slice_index=self.z.shape[0] // 2, colormap='viridis')

        # Cut off one side of the cube, leaving the other side intact
        mlab.pipeline.extract_grid(self.plot_data, extent=(
            0, self.x.shape[0], 0, self.y.shape[1], 0, self.z.shape[2] // 2))
